...
class SubscriptionForm(npyscreen.ActionForm):  # Changed to ActionForm
    def create(self):
        # Title
        self.add(npyscreen.FixedText, value="Subscription Plans", editable=False, color="STANDOUT")
    ...
    def on_ok(self):
        ...
        if not choice:
            npyscreen.notify_confirm("Please select a plan or press 'Cancel' to go back.", title="No Selection")
...
class PostPlanOptionsForm(npyscreen.ActionForm):  # Changed to ActionForm
    ...
    def on_ok(self):
        ...
        if not choice:
            npyscreen.notify_confirm("Please select an option or press 'Cancel' to go back.", title="No Selection")
            self.parentApp.switchForm('POST_PLAN')
            return
        selected = choice[0]
        if selected == 0:
            npyscreen.notify_confirm(f"Thank you for choosing to buy the {self.selected_plan}!",
...
class ClassBundleForm(npyscreen.ActionForm):  # Changed to ActionForm
    def create(self):
        # Title
        self.add(npyscreen.FixedText, value="Class Bundle Options", editable=False, color="STANDOUT")
    ...
    def on_ok(self):
        ...
        if not choice:
            npyscreen.notify_confirm("Please select a bundle or press 'Cancel' to go back.", title="No Selection")
            self.parentApp.switchForm('CLASS_BUNDLE')
            return
        selected = choice[0]
